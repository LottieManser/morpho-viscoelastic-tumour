function output = runSimViscoManser(params, progressFlag)
    ...
    drdts = zeros(params.nT,params.nR); drdts(1,:) = 0;
    growthStretches = zeros(params.nT,params.nR); growthStretches(1,:) = 1;
    dgrowthStretchesdt = zeros(params.nT,params.nR); dgrowthStretchesdt(1,:) = 0;
    ...
    for tInd = 1 : params.nT
        ...
        if tInd ~= 1
            drdts(tInd,:) = (rs(tInd,:) - rs(tInd-1,:)) / (ts(tInd) - ts(tInd-1));
            dgrowthStretchesdt(tInd,:) = (growthStretches(tInd,:) - growthStretches(tInd-1,:)) / (ts(tInd) - ts(tInd-1));
        end
        

        % Compute the radial, hoop, and bulk stresses in the current configuration.
        [radialStresses(tInd,:), hoopStresses(tInd,:), bulkStresses(tInd,:), viscoelasticStretches(tInd,:)] = ...
                computeStressesViscoManser(RsMinusB(tInd,:),rs(tInd,:),drdts(tInd,:),growthStretches(tInd,:),dgrowthStretchesdt(tInd,:),params);
        ...
        if tInd < params.nT
            ...
            % Map current quantities onto the new discretisation, storing them temporarily.
            ...
            % Carry forward the remapped derivatives for the next step
            drdts(tInd+1,:)              = drdtRemapped;
            dgrowthStretchesdt(tInd+1,:) = dgrowthStretchesdtRemapped;  % finite difference will be updated below

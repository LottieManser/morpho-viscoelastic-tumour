function output = runSimViscoManser(params, progressFlag)
    ...
    growthStretches = zeros(params.nT, params.nR);
    growthStretches(1,:) = 1;

    dgrowthStretchesdt = zeros(params.nT, params.nR);
    dgrowthStretchesdt(1,:) = 0;
    ...
    mus = zeros(params.nT, params.nR);
    mus(1,:) = params.mu0;

    varsigmas = zeros(params.nT, params.nR);
    varsigmas(1,:) = params.varsigma0;
    ...
    for tInd = 1:params.nT
        ...
        rs(tInd,:) = computeRadialCoord( ...
            RsMinusB(tInd,:), growthStretches(tInd,:), params);

        %% Update material parameters
        switch params.materialmodel
            case 'static'
                [mu_now, varsigma_now] = staticMaterialProperties(params);
                mus(tInd,:) = mu_now * ones(1, params.nR);
                varsigmas(tInd,:) = varsigma_now * ones(1, params.nR);

            case 'autonomous'
                [mu_now, varsigma_now] = ...
                    autonomousMaterialProperties(ts(tInd), params);
                mus(tInd,:) = mu_now * ones(1, params.nR);
                varsigmas(tInd,:) = varsigma_now * ones(1, params.nR);

            case 'dynamic'
                [mus(tInd,:), varsigmas(tInd,:)] = ...
                    dynamicMaterialProperties(densities(tInd), params);
        end
        ...
        [radialStresses(tInd,:), ...
         ...
         viscoelasticStretches(tInd,:)] = ...
            computeStressesViscoManser( ...
                ...
                rs(tInd,:), ...
                drdts(tInd,:), ...
                growthStretches(tInd,:), ...
                dgrowthStretchesdt(tInd,:), ...
                mus(tInd,:), ...
                varsigmas(tInd,:), ...
                params);
        ...
        %% Necrotic core
        necroticRadii(tInd) = computeNecroticRadius( ...
            necroticRadii(1:tInd-1), ...
            rs(tInd,:), ...
            nutrients(tInd,:), ...
            params);
        ...
        switch params.model
            case 'A'
                growthRates(tInd,:) = ...
                    modelAGrowthRate(nutrients(tInd,:), params);

            case 'B'
                growthRates(tInd,:) = ...
                    modelBGrowthRate(nutrients(tInd,:), rs(tInd,:), ...
                                     necroticRadii(tInd), ...
                                     radialStresses(tInd,:), params);

            case 'B2'
                growthRates(tInd,:) = ...
                    modelB2GrowthRate(nutrients(tInd,:), rs(tInd,:), ...
                                      necroticRadii(tInd), ...
                                      radialStresses(tInd,:), params);

            case 'C'
                growthRates(tInd,:) = ...
                    modelCGrowthRate(nutrients(tInd,:), rs(tInd,:), ...
                                     necroticRadii(tInd), ...
                                     radialStresses(tInd,:), params);

            case 'D'
                growthRates(tInd,:) = ...
                    modelDGrowthRate(nutrients(tInd,:), rs(tInd,:), ...
                                     necroticRadii(tInd), ...
                                     radialStresses(tInd,:), ...
            ...
            case 'E'
                growthRates(tInd,:) = ...
                    modelEGrowthRate(nutrients(tInd,:), rs(tInd,:), ...
                                     necroticRadii(tInd), ...
                                     radialStresses(tInd,:), ...
                                     hoopStresses(tInd,:), params);
        end

        %% Density (diagnostic only)
        densities(tInd,:) = 1 ./ (growthStretches(tInd,:).^3);

        %% Advance to next time step
        if tInd < params.nT

            RsMinusB(tInd+1,:) = interp1( ...
                rs(tInd,:), RsMinusB(tInd,:), ...
                linspace(rs(tInd,1), rs(tInd,end), params.nR));

            rRemapped = interp1( ...
                RsMinusB(tInd,:), rs(tInd,:), RsMinusB(tInd+1,:));

            growthStretchRemapped = interp1( ...
                RsMinusB(tInd,:), growthStretches(tInd,:), ...
                RsMinusB(tInd+1,:));

            growthRateRemapped = interp1( ...
                RsMinusB(tInd,:), growthRates(tInd,:), ...
                RsMinusB(tInd+1,:));

            drdtRemapped = interp1( ...
                RsMinusB(tInd,:), drdts(tInd,:), ...
                RsMinusB(tInd+1,:));

            dgrowthStretchesdtRemapped = interp1( ...
                RsMinusB(tInd,:), dgrowthStretchesdt(tInd,:), ...
                RsMinusB(tInd+1,:));

            drdts(tInd+1,:) = drdtRemapped;
            dgrowthStretchesdt(tInd+1,:) = dgrowthStretchesdtRemapped;

            dt = ts(tInd+1) - ts(tInd);

            growthStretches(tInd+1,:) = ...
                growthStretchRemapped + ...
                dt * growthStretchRemapped .* growthRateRemapped;

            dgrowthStretchesdt(tInd+1,:) = ...
                (growthStretches(tInd+1,:) - growthStretches(tInd,:)) / dt;
        end
    end
